...
class MetricParameterRepoTest {
    ...
    fun `findById returns parameter if found`() {
        ...
        every { resultSet.getInt("MetricParameterID") } returns 1
        ...
        val repo = repos.MetricParameterRepository(connection)
        val result = repo.findById(1)
        assertNotNull(result)
        assertEquals(1, result?.metricParameterId)
    ...
    @Test
    fun `findByMetricId returns list of parameters`() {
        every { connection.prepareStatement(any()) } returns statement
        every { statement.setInt(any(), any()) } just Runs
        every { statement.executeQuery() } returns resultSet
        every { resultSet.next() } returnsMany listOf(true, true, false)
        every { resultSet.getInt("MetricParameterID") } returnsMany listOf(1, 2)
    ...
    @Test
    fun `findByMetricIdAndName returns parameter if found`() {
        every { connection.prepareStatement(any()) } returns statement
        every { statement.setInt(1, 10) } just Runs
        every { statement.setString(2, "param") } just Runs
        every { statement.executeQuery() } returns resultSet
        every { resultSet.next() } returns true
        every { resultSet.getInt("MetricParameterID") } returns 3
        every { resultSet.getString("ParameterName") } returns "param"
        every { resultSet.getString("ParameterType") } returns "Bool"
        every { resultSet.getInt("MetricMetricID") } returns 10

        val repo = repos.MetricParameterRepository(connection)
        val result = repo.findByMetricIdAndName(10, "param")
        assertNotNull(result)
        assertEquals(3, result?.metricParameterId)
        assertEquals("param", result?.parameterName)
    ...
    fun `save inserts parameter and returns generated id`() {
        ...
        val repo = repos.MetricParameterRepository(connection)
        val id = repo.save(MetricParameter(null, "param", "String", 10))
        assertEquals(99, id)
    }
    ...
    fun `save throws if no generated key`() {
        ...
        assertThrows(IllegalStateException::class.java) {
            repo.save(MetricParameter(null, "param", "String", 10))
        }
    }
    ...
    fun `update updates parameter type`() {
        ...
        val repo = repos.MetricParameterRepository(connection)
        repo.update(MetricParameter(5, "param", "Int", 10))
